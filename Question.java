...
public class Question {
	...
	public Question(int num, Backend back) {
		boolean fit = false;
		...
		right = (int) (Math.random() * back.getDifficulty());
		for (int i = 0; i < back.getDifficulty(); i++) variants.add(-1); //so variants collection does not contain variants before the selection
		variants.set(right, num);

		for (int i = 0; i < back.getDifficulty(); i++) {
			if (variants.get(i) == num) continue;
			do {
				do {
					variants.set(i, (int) (Math.random() * back.getRange()));
